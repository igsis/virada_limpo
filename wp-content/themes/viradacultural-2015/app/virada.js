...
app.controller('programacao', function($scope, $rootScope, $http, $location, $timeout, $window, DataService){
    ...
    $scope.$watch('data', function(oldValue, newValue){
        ...
        timeouts.setHash = $timeout(function(){
            ...
            if(oldValue.searchText !== newValue.searchText || oldValue.searchRegion !== newValue.searchRegion){
				document.getElementById("btnRegion").innerText = ($scope.data.searchRegion == '' ? 'zona' : $scope.data.searchRegion);
    ...
    $scope.populateEntities = function(delay){
        ...
        timeouts.populateEntities = $timeout(function(){
			...
			var region = $scope.data.searchRegion;
            ...
            $scope.eventIndex.forEach(function(event){
				...
				if(space.geoZona == null) {
					console.log(space)
					space.geoZona = '';
				}
				
                if(event && (txt.trim() === '' || event.text.indexOf(txt) >= 0 || (space && $scope.unaccent(space.name).indexOf(txt) >=0 ) ) 
					&& (event.startsAt <= searchEndsAt &&  event.startsAt >= searchStartsAt || event.entity.duration === '24h00')
					&& space.geoZona.toLowerCase().indexOf(region.toLowerCase()) >= 0){
    ...
    $scope.renderList = function(){
        ...
        timeouts.renderList = $timeout(function(){
            renderingList = true;
            var spacesPerPage = 10000;
            var eventsPerPage = 1000000;
            var offset;
